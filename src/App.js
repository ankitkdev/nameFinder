...
function App() {
  ...
  const searchByRecipe = () => {
    setIsLoading(true);
    fetch(`https://api.nationalize.io/?name=${selectedRecipe}`)
      .then((response) => {
    ...
    <>
      <div className="container mx-auto mt-2">
        ...
        <div className="row flex mt-5 justify-center">
          ...
          <button type="button" onClick={Revese} class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 m-2 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Sort</button>
        </div>
        {isLoading && (
          <div role="status" className="row flex mt-3 justify-center">
        ...
        {recipe.length <= 0 && (
          <div className="row flex mt-5 justify-center">
            <div class="bg-white image-width border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              ...
              <div class="p-5">
                <a href="#">
                  <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">No Name</h5>
                </a>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 m-3">
          {recipe.map((recipe) => {
            return (
              ...
              <div class="bg-white image-width m-2 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <div class="p-5">
                  <a href="#">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{recipe.country_id}</h5>
                  </a>
                  <p class="mb-3 font-normal text-yellow-700 dark:text-gray-400">{((recipe.probability) * 100).toFixed(2)} %</p>
                </div>
              </div>
            )
          })}
